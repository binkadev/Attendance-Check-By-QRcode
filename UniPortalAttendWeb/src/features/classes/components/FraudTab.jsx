...
import {
  ...
  Server, Calendar, FileText, UserX
} from 'lucide-react';
...
const EVIDENCE_KEY_MAP = {
  lastFailureCode: 'Mã lỗi cuối cùng',
  ipAddress: 'Địa chỉ IP',
  location: 'Tọa độ GPS',
  deviceId: 'Mã định danh thiết bị',
  userAgent: 'Thiết bị/Trình duyệt',
  distance: 'Khoảng cách (m)',
  timeThreshold: 'Ngưỡng thời gian',
  scanCount: 'Số lần quét',
  occurrenceCount: 'Số lần vi phạm',
  confidenceScore: 'Độ tin cậy',
  distanceToClassroom: 'Khoảng cách đến lớp học',
  timeBetweenScans: 'Thời gian giữa các lần quét',
  sharedUsersCount: 'Số người dùng chung thiết bị',
  burstCount: 'Số lượng yêu cầu liên tục',
  failedAttempts: 'Số lần thử thất bại',
  recentIPs: 'Các IP gần đây',
...
const MOCK_INCIDENTS = [
  {
    ...
    description: 'Thiết bị A8F2 được dùng để điểm danh cho 2 sinh viên khác nhau',
    student: {
      id: 'mock-user-1',
      name: 'Nguyễn Văn Mạnh',
      studentCode: 'STU-6520',
      avatar: 'https://i.pravatar.cc/150?u=mich'
    },
    lastDetectedAt: new Date(Date.now() - 2 * 3600 * 1000).toISOString(),
    severity: 'HIGH',
    confidence: 98,
    status: 'OPEN',
    evidenceSummary: {
  ...
  },
  {
    ...
    student: {
      id: 'mock-user-2',
      name: 'Lê Thị Hồng',
      studentCode: 'STU-4421',
      avatar: 'https://i.pravatar.cc/150?u=sarah'
    },
    lastDetectedAt: new Date(Date.now() - 5 * 3600 * 1000).toISOString(),
    severity: 'MEDIUM',
    confidence: 75,
    status: 'OPEN',
    evidenceSummary: {
      distanceToClassroom: 2400,
      location: { lat: 21.037, lng: 105.783 }
    }
  },
  {
    id: 'MOCK-003',
    type: 'IP_BURST_MULTI_ATTEMPT',
    title: 'Quét QR quá nhanh',
    description: 'Phát hiện 3 lần quét trong 4 giây từ cùng một IP',
    student: {
      id: 'mock-user-3',
      name: 'Trần Đăng Khoa',
      studentCode: 'STU-1198',
      avatar: 'https://i.pravatar.cc/150?u=david'
    },
    lastDetectedAt: new Date(Date.now() - 24 * 3600 * 1000).toISOString(),
    severity: 'CRITICAL',
    confidence: 92,
    status: 'ACKNOWLEDGED',
    evidenceSummary: {
...
export default function FraudTab({ classId }) {
  ...
  const handleResolve = async (incidentId) => {
    ...
    if (String(incidentId).startsWith('MOCK-')) {
              ...
              <tbody className="divide-y divide-gray-100">
                {incidents
                  .filter(inc => {
                    ...
                    const studentCode = inc.student?.studentCode || '';
                    const name = inc.student?.name || '';
                    return studentCode.toLowerCase().includes(searchStr) || name.toLowerCase().includes(searchStr);
                  })
                  .map((incident) => (
                    <tr key={incident.id} className={`hover:bg-gray-50 transition-colors group ${incident.status === 'RESOLVED' ? 'opacity-60 bg-gray-50/30' : ''}`}>
                      ...
                      <td className="p-4 flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 max-w-xs truncate" title={incident.description}>
                            {incident.description || 'Phát hiện theo logic hệ thống'}
                          </p>
                        </div>
                      </td>
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-bold text-gray-900 text-[13px]">{incident.student?.name || 'KHÔNG RÕ'}</p>
                            <p className="text-xs text-gray-500 truncate w-24" title={incident.student?.studentCode}>{incident.student?.studentCode || 'Không rõ MSSV'}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      {/* Thời gian */}
                      <td className="p-4">
                        <p className="font-bold text-gray-900 text-[13px]">{formatDate(incident.lastDetectedAt)}</p>
                        <p className="text-xs text-gray-500">{formatDay(incident.lastDetectedAt)}</p>
                      </td>
                      ...
                      <td className="p-4 align-middle">
                        ...
                        {incident.status === 'OPEN' && (
                          <div className="mt-1 flex items-center gap-1 text-[10px] text-amber-600 font-bold">
                            <Timer size={10} /> Đang chờ
                          </div>
                        )}
                        {incident.status === 'ACKNOWLEDGED' && (
                          <div className="mt-1 flex items-center gap-1 text-[10px] text-blue-600 font-bold">
                            <Info size={10} /> Đã ghi nhận
                          </div>
                        )}
                        {incident.status === 'FALSE_POSITIVE' && (
                          <div className="mt-1 flex items-center gap-1 text-[10px] text-gray-500 font-bold">
                            <X size={10} /> Báo cáo sai
                          </div>
                        )}
                      </td>
              ...
              <div className="flex gap-4 items-start">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    ) : selectedDetail.status === 'ACKNOWLEDGED' ? (
                      <span className="text-[10px] font-bold text-blue-600 bg-blue-100 px-2 py-0.5 rounded-full flex items-center gap-1">
                        <Info size={12} /> Đã ghi nhận
                      </span>
                    ) : selectedDetail.status === 'FALSE_POSITIVE' ? (
                      <span className="text-[10px] font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full flex items-center gap-1">
                        <X size={12} /> Báo cáo sai
                      </span>
                    ) : (
                      <span className="text-[10px] font-bold text-gray-600 bg-gray-100 px-2 py-0.5 rounded-full flex items-center gap-1">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              ...
              <div>
                ...
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-bold text-gray-900">{selectedDetail.student?.name || 'Không rõ'}</p>
                    <p className="text-sm text-gray-500 font-mono mt-0.5">{selectedDetail.student?.studentCode || 'Không rõ MSSV'}</p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex px-4 py-3 gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm font-semibold text-gray-900">
                        {formatDate(selectedDetail.lastDetectedAt)} - {formatDay(selectedDetail.lastDetectedAt)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex px-4 py-3 gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <pre className="text-xs font-mono text-gray-800 bg-gray-50 p-3 rounded-lg border border-gray-100 overflow-x-auto">
                        {selectedDetail.evidenceSummary && Object.keys(selectedDetail.evidenceSummary).length > 0
                          ? JSON.stringify(selectedDetail.evidenceSummary, null, 2)
                          : `{}`}
                      </pre>
                    </div>
                  </div>
